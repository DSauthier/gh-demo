...
// Create order (checkout) - SECURE: Calculates total server-side
app.post('/api/checkout', (req, res) => {
  // Get cart items to calculate real total server-side
  const cartQuery = `
  ...
  db.all(cartQuery, (err, cartItems) => {
    ...
    // Calculate total SERVER-SIDE from cart items
    const serverCalculatedTotal = cartItems.reduce((sum, item) => sum + (item.quantity * item.price), 0);
