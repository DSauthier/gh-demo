...
// Create order (checkout) - FIXED: Server-side total calculation
app.post('/api/checkout', (req, res) => {
  // Get cart items and calculate total server-side
  const cartQuery = `
  ...
  db.all(cartQuery, (err, cartItems) => {
    ...
    // SECURITY FIX: Calculate total server-side from cart items
    const serverCalculatedTotal = cartItems.reduce((sum, item) => 
      sum + (item.quantity * item.price), 0
    );

    // Business logic validation: reject invalid totals
