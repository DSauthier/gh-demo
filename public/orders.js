...
async function loadOrders() {
    try {
        ...
        if (orders.length === 0) {
            const noOrdersMsg = typeof t === 'function' ? 
                `${t('orders.no_orders')}<a href="index.html">${t('orders.no_orders_link')}</a>${t('orders.no_orders_end')}` :
        ...
        const errorMsg = typeof t === 'function' ? 
            t('messages.error_loading_orders') : 'Error loading orders:';
...
function updateStats(orders) {
    ...
    const formattedRevenue = typeof formatCurrency === 'function' ? 
        formatCurrency(totalRevenue) : `$${totalRevenue.toFixed(2)}`;
    const formattedLostRevenue = typeof formatCurrency === 'function' ? 
        formatCurrency(estimatedLostRevenue) : `$${estimatedLostRevenue.toFixed(2)}`;
